...
var is_web_or_compat: bool = OS.has_feature("web") or RenderingServer.get_rendering_device() == null

var DEFAULTS := {
	video = {
		display_mode = Window.MODE_WINDOWED if (OS.has_feature("web") or OS.has_feature("mobile")) else Window.MODE_EXCLUSIVE_FULLSCREEN,
		vsync = DisplayServer.VSYNC_ENABLED,
		max_fps = 0,
		resolution_scale = 1.0,
		scale_filter = Viewport.SCALING_3D_MODE_BILINEAR if is_web_or_compat else (Viewport.SCALING_3D_MODE_METALFX_TEMPORAL if metalfx_supported else Viewport.SCALING_3D_MODE_FSR2),
	},
	rendering = {
		taa = false,
		...
		shadow_mapping = true,
		gi_type = GIType.LIGHTMAP_GI if is_web_or_compat else GIType.VOXEL_GI,
		gi_quality = GIQuality.DISABLED if is_web_or_compat else GIQuality.LOW,
		ssao_quality = -1 if is_web_or_compat else RenderingServer.ENV_SSAO_QUALITY_MEDIUM,
		ssil_quality = -1,  # Disabled
		bloom = false if OS.has_feature("web") else true,
		volumetric_fog = false if is_web_or_compat else true,
	},
}
