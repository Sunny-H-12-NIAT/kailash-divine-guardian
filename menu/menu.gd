...
func _ready() -> void:
	...
	if player_model:
		var anim = player_model.get_node_or_null("AnimationPlayer")
		if anim:
					...
					for t in range(a.get_track_count()):
						...
						if "UpperArm.L" in path:
							for k in range(a.track_get_key_count(t)):
								var val = a.track_get_key_value(t, k)
								if val is Quaternion:
									a.track_set_key_value(t, k, q_down_l * val)
						elif "UpperArm.R" in path:
							for k in range(a.track_get_key_count(t)):
								var val = a.track_get_key_value(t, k)
								if val is Quaternion:
									a.track_set_key_value(t, k, q_down_r * val)
			if anim.has_animation(&"Idle_Standing"):
