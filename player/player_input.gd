...
func _process(delta: float) -> void:
	...
	if shooting or aiming:
		var ch_pos = get_viewport().get_visible_rect().size * 0.5
		if crosshair:
			ch_pos = crosshair.get_global_rect().get_center()
		var ray_from = camera_camera.project_ray_origin(ch_pos)
		var ray_dir = camera_camera.project_ray_normal(ch_pos)

		var parent_node = get_parent()
		var exclude_arr: Array = [self]
		if parent_node:
			exclude_arr.append(parent_node)
		var col = parent_node.get_world_3d().direct_space_state.intersect_ray(PhysicsRayQueryParameters3D.create(ray_from, ray_from + ray_dir * 1000.0, 0b11, exclude_arr))
		if col.is_empty():
